...
public class AdviseController {
    ...
    public ResponseEntity list(@RequestParam(value = "start", required = false, defaultValue = "0") Integer start,
        ...
        if (StringUtils.isNotEmpty(studentId) || StringUtils.isNotEmpty(facultyId) || StringUtils.isNotEmpty(info)) {
            message = adviseService.advisingList(String.valueOf(start * length - 1), String.valueOf(length), search, order, orderCol, studentId, facultyId, info);
    ...
    public ResponseEntity list(@RequestParam(value = "start", required = false, defaultValue = "0") Integer start,
        ...
        search = search.replace("'", "");
        ResultMap message = adviseService.advisorList(String.valueOf(start * length - 1), String.valueOf(length), search, order, orderCol, status);
        message.put("recordsTotal", ((Page) message.get("page")).getTotalRows());
        return new ResponseEntity<>(message, HttpStatus.OK);
    }
